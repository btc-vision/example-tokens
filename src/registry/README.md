...
A decentralized package registry for OPNet plugins. Manages package ownership, version metadata, and deprecation status entirely on-chain with IPFS for binary storage.
...
After 72 hours, the version becomes **fully immutable**. This protects downstream consumers while giving publishers time to catch mistakes.
...
Variable-length strings (IPFS CIDs, version ranges, deprecation reasons) use `AdvancedStoredString` which spans multiple 32-byte storage slots.
